...
const englishCopy = {
  home: [
    ...
    [".home-manifesto__eyebrow", "Born in Benin, Les Patronnes aims to grow across West Africa and then throughout the continent."],
...
(() => {
  ...
  backgroundVideos.forEach((video) => {
    ...
    ["loadeddata", "canplay", "ended"].forEach((eventName) => video.addEventListener(eventName, () => resume(video)));
    video.addEventListener("pause", () => window.setTimeout(() => resume(video), 250));
    resume(video);
  });

  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) backgroundVideos.forEach(resume);
  });
})();
